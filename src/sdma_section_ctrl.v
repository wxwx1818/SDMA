...
always@(*)begin
	case(cur_state)
		...
		TRANSFER:begin
			num_of_remain_bytes_next = (i_ssc_srcfmsmovelength - sfms1_transfer_cnt > `SDMA_CACHEDATAWIDTH/8)?(`SDMA_CACHEDATAWIDTH/8):(i_ssc_srcfmsmovelength - sfms1_transfer_cnt);
		end
		CONCATETRANSFER:begin
			if(~o_ssc_concate_fms_switch_flag)begin
				num_of_remain_bytes_next = (i_ssc_srcfms2concatelength - sfms2_concate_cnt > `SDMA_CACHEDATAWIDTH/8)?(`SDMA_CACHEDATAWIDTH/8):(i_ssc_srcfms2concatelength - sfms2_concate_cnt);
			end
			else begin
				num_of_remain_bytes_next = (i_ssc_srcfms1concatelength - sfms1_concate_cnt > `SDMA_CACHEDATAWIDTH/8)?(`SDMA_CACHEDATAWIDTH/8):(i_ssc_srcfms1concatelength - sfms1_concate_cnt);
			end
		end
		default:begin
			num_of_remain_bytes_next = {(`SDMA_SECTION_DINNUMDATAWIDTH){1'b0}};
